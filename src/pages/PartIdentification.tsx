...
const PartIdentificationComponent = ({ history }: { [key: string]: any }) => {
  ...
  const uploadImage = () => {
    setLoading(true);
    storageRef.child('images').put(rawImage).then(function(snapshot) {
      storageRef.child('images').getDownloadURL().then(URL => getIdentification(URL))
    });
  ...
  const renderResultList = (result:any, index:any) => {
    if(result.score > 0.9) {
      return(
        ...
        <IonImg style={{width: "200px", height: "200px"}} src={"/"+result.class+".JPG"}/>
        ...
        <IonGrid>
          <IonRow>
            ...
            <IonCol>
              ...
              <IonRow class="results">{loading? "Loading..." : resultList.map(renderResultList)}{resultList.map(renderResultList)}{resultList.map(renderResultList)}{resultList.map(renderResultList)}{resultList.map(renderResultList)}{resultList.map(renderResultList)}</IonRow>
            </IonCol>
          </IonRow>
        </IonGrid>
      </IonContent>
